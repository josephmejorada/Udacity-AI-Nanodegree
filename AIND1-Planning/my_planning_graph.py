...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Get previous state level
        s_level_prev = self.s_levels[level]
        # List all available actions
        actions = self.all_actions
        # Initialize a new level of actions
        self.a_levels.append(set())
        for action in actions:
            a_node = PgNode_a(action)
            if a_node.prenodes.issubset(s_level_prev):
                # Establishes a connection between the action and state levels
                for s_node in s_level_prev:
    ...
    def add_literal_level(self, level):
        ...
        a_level_prev = self.a_levels[level - 1]
        # Initialize a new level of states
        self.s_levels.append(set())
        for a_node in a_level_prev:
            # Establishes a connection between action level and effect of the action
            for effect_node in a_node.effnodes:
                a_node.children.add(effect_node)
                effect_node.parents.add(a_node)
                # Add a state to the level
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        check_symbol = node_s1.symbol == node_s2.symbol
        check_negation = node_s1.is_pos != node_s2.is_pos
        return check_symbol and check_negation
